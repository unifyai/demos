...
# ToDo: replace with simpler code once batched log creation is supported
def save_dataset():
    logs = list()
    example_id = 0
...
dataset_10 = random.sample(dataset, 10)

agent = unify.Unify("o3-mini@openai", traced=True)

if os.path.exists(".cache.json"):
    os.remove(".cache.json")
wget.download("https://raw.githubusercontent.com/unifyai/demos/refs/heads/main/ai_tutor/.cache.json")
...
def update_markscheme(markscheme: str):
    m_marks = list(set(re.findall(r'M\d+', markscheme)))
    a_marks = list(set(re.findall(r'A\d+', markscheme)))
    b_marks = list(set(re.findall(r'B\d+', markscheme)))
    sc_marks = list(set(re.findall(r'SC\d+', markscheme)))
